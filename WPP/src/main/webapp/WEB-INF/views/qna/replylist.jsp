...
function showReplyComment(rnum) {
	$.ajax({
		type : "get",
		url : "${path}/qnareply/commentwrite/" + rnum,
		success : function(result) {
		...
		<div style= "margin-left: <c:out value="${30*row.reindent}"/>px; border:1px solid; background-color:white">
         ...
         `&emsp;
          ...
          <c:if test="${sessionScope.userId == row.replyer}">
          	<button type="button" class="btn btn-default" onClick="location.href='/qnareply/delete?rnum=${row.rnum}&bnum=${row.bnum}'">삭제 </button>
